...
def get_github_repo_file_list(settings, log_queue):
    # ⭐️ URL 생성 시 하드코딩된 'main' 대신 설정값을 사용
    ...
    try:
        ...
        # 404 오류는 브랜치가 없거나(저장소가 비었거나), 저장소가 없는 경우입니다.
        # 이 경우를 오류로 처리하지 않고, 파일이 없는 상태로 간주합니다.
        if e.response.status_code == 404:
            log_queue.put("ℹ️ 깃허브 저장소 또는 브랜치를 찾을 수 없습니다. (빈 저장소일 수 있음)")
            return set() # 빈 세트를 반환하여 '파일 없음'으로 처리
        else:
...
def move_file_to_recycle_bin(repo_path, settings, log_queue):
    """지정된 경로의 파일을 깃허브의 _recycle_bin 폴더로 이동시킵니다."""
    log_queue.put(f"- 처리 대상 (휴지통 이동): {os.path.basename(repo_path)}")
    
    # 1. 원본 파일의 내용과 sha 값을 가져옵니다.
    ...
    try:
        ...
        if response_put.status_code not in [200, 201]:
             ...
             return # 복사 실패 시 원본을 삭제하지 않음
    except Exception as e:
        log_queue.put(f"  ❌ 네트워크 오류 (휴지통 생성 중): {e}")
        return

    # 4. 휴지통으로 복사가 성공했을 때만 원본 파일을 삭제합니다.
    del_url = get_url # 삭제 URL은 조회 URL과 동일
...
class App:
    def __init__(self, root):
        ...
        self.btn_start = ttk.Button(control_frame, text="동기화 및 업로드 시작", command=self.start_action, bootstyle="success")
        self.btn_start.grid(row=0, column=0, sticky="ew", padx=(0, 5), ipady=10)
        self.btn_stop = ttk.Button(control_frame, text="업로드 종료", state="disabled", command=self.stop_action, bootstyle="danger")
